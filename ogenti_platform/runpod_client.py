...
def process_completed_job(
    ...
    Process RunPod job output:
      ...
      2. Encrypt to .ogt format
      3. Create Adapter record
      4. Update TrainingJob status

    Args:
        runpod_output: RunPod output dict with 'adapter_base64', 'metrics', etc.
        job_id: Our TrainingJob ID
        user_id: User ID
        model: Model key (e.g. 'qwen2.5-3b')
    ...
    try:
        ...
        adapter_name = f"{model_label} Adapter #{job_id}"
        ...
        logger.info(f"Job {job_id} completed: adapter={adapter.id}, size={adapter.file_size}")
        return {
