...
> *Quick 테스트는 끝났다. 이제 진짜 돌릴 시간이다.*

---

## 목차

- [Quick vs Production — 뭐가 다른데?](#-quick-vs-production--뭐가-다른데)
- [필요한 것들](#-필요한-것들)
...
우리가 아까 돌린 `--quick`은 말 그대로 **"되긴 하네?"** 확인용이었다.

```
                   Quick Test              Production
                   ─────────              ──────────
모델             Qwen2.5-0.5B           Qwen2.5-3B-Instruct
디바이스          CPU                     A100/H100 GPU
에피소드          100                     58,000
Phase 0          25 에피소드              5,000 에피소드
Phase 4(KD)      10 에피소드              8,000 에피소드
학습시간          ~2시간(CPU)             ~18-24시간(A100)
정밀도            float32                 bf16 (Mixed Precision)
DeepSpeed         없음                    ZeRO-2
W&B 로깅          비활성                   활성
체크포인트         50 에피소드마다          1,000 에피소드마다
Eval              25 에피소드마다          500 에피소드마다
배치사이즈         4                       32 → 16 → 8 → 8 → 4

결과              "프로토콜 냄새"          진짜 프로토콜 탄생
```

**Quick에서 본 것:**
- 리워드가 0.11에서 0.26까지 오르는 걸 봤다
- 압축률이 0.2x에서 1.6x까지 찍는 걸 봤다
- Phase 0 → Phase 1 전환을 봤다

**Production에서 볼 것:**
- 리워드가 0.8+ 까지 올라간다
- 압축률이 **15x** 까지 올라간다 (45토큰 → 3토큰)
- AI가 **자기만의 언어를 발명**한다
- 그 언어를 다른 AI에 이식할 수 있는 **범용 어댑터**가 나온다
...
아까 Quick 테스트에서 봤잖아. accuracy가 0에서 0.135로 올라가는 거. 압축률이 1.6x 찍는 거. Phase 전환되는 거. 그게 100 에피소드 동안 맛보기였다면, 58,000 에피소드는 풀코스다.
