...
const { app, BrowserWindow, ipcMain, Tray, Menu, dialog, shell, nativeImage, safeStorage, globalShortcut, desktopCapturer } = require('electron');
...
const SAFE_KEYS = ['llmApiKey', 'jwtSecret', 'jwtRefreshSecret', 'agentRuntimeSecret',
                   'stripeSecretKey', 'stripeWebhookSecret', 'encryptionKey'];
...
function createMainWindow() {
  ...
  // Aggressively clear ALL browser caches to ensure fresh frontend assets
  ...
  Promise.all([
    ...
    debugLog(`[Cache] Partial clear: ${e.message}`);
  });
...
async function startServices() {
  ...
  const stripeKey = safeGet('stripeSecretKey');
  const stripeWebhook = safeGet('stripeWebhookSecret');
  if (stripeKey) env.STRIPE_SECRET_KEY = stripeKey;
  if (stripeWebhook) env.STRIPE_WEBHOOK_SECRET = stripeWebhook;

  // Persist generated secrets in safeStorage
  ...
  const runtimeEnv = {
    ...
    AGENT_RUNTIME_SECRET: env.AGENT_RUNTIME_SECRET,
    LOG_LEVEL: 'INFO',
  };
...
ipcMain.handle('stripe:save-keys', (_, { secretKey, webhookSecret }) => {
  if (secretKey !== undefined) {
    if (secretKey) {
      ...
      // Clear the key
      try { store.delete('safe_stripeSecretKey'); } catch(e) {}
    }
  }
  if (webhookSecret !== undefined) {
    if (webhookSecret) {
      ...
      try { store.delete('safe_stripeWebhookSecret'); } catch(e) {}
    }
  }
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await startServices();
    // Navigate to the actual frontend
    const frontendPort = store.get('frontendPort');
...
app.on('before-quit', (e) => {
  isQuitting = true;
  // Electron does NOT await async handlers in before-quit.
  // We must preventDefault, run cleanup, then force-quit.
  e.preventDefault();
  Promise.all([
    stopFrontend(frontendProcess).catch(() => {}),
    stopBackend(backendProcess).catch(() => {}),
